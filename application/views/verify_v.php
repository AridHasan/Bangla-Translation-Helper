    ...
    <script type="text/javascript">
        ...
        function validateInput()
        {
            ...
            if(hasWhiteSpaceOrEmpty(inputVal))
            {
                ...
                $("#translate2").attr("disabled", "disabled");
            }
            else
            {
                $("#translate").removeAttr("disabled");
                $("#translate2").removeAttr("disabled");
            }
        }
        ...
        <div class="row" style="margin-top: 20px;">
            <div class="col-sm">
                <div class="form-group">
                    <textarea class="form-control" rows="7" style="width:100%;" disabled><?php if(!empty($source)){echo $source;}else{echo 'Translated sentences Not found. Try Again Later';} ?></textarea>
                </div>
            </div>
            <div class="col-sm">
                <form class="form-group" method="post" action="<?php echo base_url();?>verify/verifying">
                    <div class="form-group col-sm">
                        <div class="form-group">
                            <textarea class="form-control" rows="7" id="targetText" name="targetText" placeholder="Translation" style="width:100%;"><?php if(!empty($target)){echo $target;}else{echo 'Translated sentences Not found. Try Again Later';}?></textarea>
                            <input type="hidden" name="sentence" value="<?php if(!empty($sId)){echo $sId;} ?>">
                            <input type="hidden" name="project" value="<?php if(!empty($pId)){echo $pId;} ?>">
                            <input type="hidden" name="user" value="<?php if(!empty($uId)){echo $uId;} ?>">
                            <input type="hidden" name="tId" value="<?php if(!empty($tId)){echo $tId;} ?>">
                        </div>
                    </div>
                    <div class="form-group offset-sm-4" style="margin-top: 10px;">
                        <button type="submit" class="btn btn-secondary" id="skip" name="skip" value="Skip">Skip</button>
                        <button type="submit" class="btn btn-danger" id="translate2" name="invalid" value="Submit">Invalid</button>
                        <button type="submit" class="btn btn-primary" id="translate" name="valid" value="Submit">Valid</button>
                    </div>
                </form>
            </div>
        </div>
